...
void normalizeClassName(std::string &classname) {
...
Object::Object(JVMLoader loader, std::string className)
    : HandleEnv(loader), reflect(loader), invoke(loader) {
  ...
  auto constructor = Wrapper(env->functions->GetMethodID, env, member,
                             CLASS_DEFAULT_CTS.c_str(), VOID_RETURN.c_str());

  object = Wrapper(env->functions->NewObject, env, member, constructor);
...
const std::vector<JavaMethod> &Object::GetMembers() { return methods; };
